...
router.get("/available/:day/:startTime/:endTime", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
...
router.post("/:id/certifications", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (existingCert) {
      return res.status(400).json({ message: "Employee already certified for this machine" });
    }
...
router.delete("/:id/certifications/:machineId", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    // Find the employee
    const employee = await Employee.findOne({ 
      _id: req.params.id,
...
router.get("/certified/:machineId", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    // Validate machine ID
    const machine = await Machine.findOne({ 
