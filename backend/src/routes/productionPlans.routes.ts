...
router.post("/", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    if (!name || !weekStartDate) {
      return res
        .status(400)
        .json({ message: "Name and weekStartDate are required" });
    }
    ...
    if (recipes && recipes.length > 0) {
      ...
      for (const newRecipe of recipes) {
        ...
        if (existingRecipe) {
          ...
          const difference =
            newRecipe.plannedAmount - (existingRecipe.plannedAmount || 0);

          if (difference !== 0) {
            // Update recipe's planned production
            await Recipe.findByIdAndUpdate(newRecipe.recipeId, {
      ...
      for (const existingRecipe of existingRecipes) {
        ...
        if (!stillExists) {
          ...
          await Recipe.findByIdAndUpdate(existingRecipe.recipeId?.toString(), {
  ...
  try {
    ...
    if (plan.recipes && plan.recipes.length > 0) {
      for (const recipeItem of plan.recipes) {
        await Recipe.findByIdAndUpdate(recipeItem.recipeId, {
  ...
  async (req: AuthedRequest, res: Response) => {
    try {
      ...
      if (plan.status === "completed") {
        return res
          .status(400)
          .json({ message: "Production plan is already completed" });
      }

      if (plan.status === "archived") {
        return res
          .status(400)
          .json({ message: "Archived plans cannot be completed" });
      }
      ...
      if (plan.blocks && plan.blocks.length > 0) {
        ...
        for (const block of productionBlocks) {
          if (block.recipeId) {
            ...
            await Recipe.findByIdAndUpdate(block.recipeId?.toString(), {
              $inc: {
